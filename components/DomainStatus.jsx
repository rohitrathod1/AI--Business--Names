    ...
    useEffect(() => {
        const getDomainStatus = async () => {
            if (domain) {
                ...
                try {
                    setLoading(true)
                    const response = await fetch(`https://www.whoisxmlapi.com/whoisserver/WhoisService?apiKey=${process.env.NEXT_PUBLIC_DOMAIN_STATUS_API}&domainName=${domain}&credits=DA`, {
                        method: 'get'
                    })
